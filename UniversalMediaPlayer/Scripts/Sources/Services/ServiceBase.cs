...
namespace UMP.Services
{
    public abstract class ServiceBase<T> : IService<T>, IAsyncService<T>
        where T : Video
    {
        //public IEnumerable<T> GetAllVideos(string videoUri, Action<List<Video>> resultCallback, Action<string> errorCallback = null) =>
        //    GetAllVideosAsync(videoUri).GetAwaiter().GetResult();
        //public abstract IEnumerator<T> GetAllVideosAsync(string url, Action<List<Video>> resultCallback, Action<string> errorCallback = null);
        ...
        internal abstract Task<IEnumerable<T>> GetAllVideosAsync(string videoUri, Func<string, Task<string>> sourceFactory, Action<List<Video>> resultCallback, Action<string> errorCallback = null);
        ...
        protected virtual HttpMessageHandler MakeHandler()
        {
            ...
            cookieContainer.Add(new Uri(YoutubeService.YoutubeUrl), new Cookie("CONSENT", "YES+cb", "/", ".youtube.com"));
            // Handler
